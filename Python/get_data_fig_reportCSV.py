...
if __name__ == "__main__":
    dic = {}
    base = {}
    metrics = ["ipc", "SpeedUp", "L1DAccuracyTime", "L1DAccuracyLate", "L1DCoverageTime", "L1DCoverageLate", "L1DMPKI", "L2CMPKI", "LLCMPKI"]
    ...
    with open ('detail_results.csv', 'w') as fw:
        fw.write("l1i_pf,l1d_pf,l2c_pf,bench")
        for metric in metrics:
            fw.write(",{}".format(metric))
        fw.write("\n")
        for l1i in dic.keys():
            for l1d in dic[l1i].keys():
                for l2c in dic[l1i][l1d].keys():
                    for bench in dic[l1i][l1d][l2c].keys():
                        fw.write("{},{},{},{}".format(l1i,l1d,l2c,bench))
                        for metric in metrics:
                            if metric in dic[l1i][l1d][l2c][bench].keys():
                                fw.write(",{}".format(dic[l1i][l1d][l2c][bench][metric]))
                            else:
                                fw.write(",0")
                        fw.write("\n")
