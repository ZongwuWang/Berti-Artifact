...
//#define PUSH_PREFETCHES_FROM_L2_TO_L1	//Neelu: Adding to L1 PQ after filling
//prefetch in L2
        ...
        if ((MSHR.entry[mshr_index].instruction == 0) &&
          ...
          if (warmup_complete[fill_cpu])
            if (ooo_cpu[fill_cpu].num_retired -
              ...
              l2c_mpki = (ooo_cpu[fill_cpu].L2C.sim_miss[fill_cpu][0] * 1000) /
                         (ooo_cpu[fill_cpu].num_retired);
          /*			if((((temp_metadata >> 17) & 1) | ((temp_metadata >> 18)
             & 1)) == 1) getting_hint_from_l2++;*/
  ...
  for (uint32_t i = 0; i < MAX_READ; i++) {
    ...
    if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) &&
      ...
      if (cache_type == IS_L2C &&
        ...
        //	    way = 1;
        //@v - dumping translation entries of DTLB with cycle number and
        //hit/miss
        /*
           if (warmup_complete[read_cpu] && cache_type==1 ) {
           cout <<  "no" << counter << ","  << hex << RQ.entry[index].full_addr
           << "," << dec << RQ.entry[index].event_cycle << "," ; if(way >=0)
           cout << "1" << endl; else cout << "0" << endl; counter++;
           }
           */
