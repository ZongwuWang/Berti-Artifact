...
double cache_data_tag_accesses[7][2000] = {0},
       cache_tag_accesses[7][2000] = {0}, dram_accesses[2000] = {0},
       interconnect_request[2000] = {0}, interconnect_response[2000] = {0};

#ifdef IPCP_PREFETCHER
double ip_table_write[2000] = {0}, ip_table_read[2000] = {0},
       ip_table_tag_write[2000] = {0}, ip_table_tag_read[2000] = {0};
...
extern uint64_t ip_table_write_accesses, ip_table_read_accesses,
    rstable_write_accesses, rstable_read_accesses, cspt_write_accesses,
    cspt_read_accesses, rrfilter_read_accesses;
extern uint64_t ip_table_tag_write_accesses, ip_table_tag_read_accesses,
...
void print_energy_profile() {
  for (int i = 0; i < phase_id; i++) {
    ...
    for (int j = 0; j < 7; j++) {
      if (i > 0) {
        total +=
            cache_data_tag_accesses[j][i] - cache_data_tag_accesses[j][i - 1];
...
void print_roi_stats(uint32_t cpu, CACHE *cache) {
  ...
  if (cache->roi_access[cpu][0]) {
    ...
    cout << " LOAD      ACCESS: " << setw(10) << cache->roi_access[cpu][0]
         ...
         << endl; // << " T_ACCESS: " << setw(10) << cache->ACCESS[0] << "
                  // T_HIT: " << setw(10) << cache->HIT[0] << " T_MISS: " <<
                  // setw(10) << cache->MISS[0] << " T_MSHR_MERGED: " <<
                  // cache->MSHR_MERGED[0] << endl; //@Vishal: MSHR merged will
                  // give correct result for 1 core, multi_core will give whole
                  // sim

    if (cache->cache_type == IS_L2C) {
      // Neelu: Instruction and Data loads.
      cout << cache->NAME << " DATA LOAD MPKI: "
           << ((double)(cache->roi_miss[cpu][0] -
                        cache->roi_instr_miss[cpu][0]) *
               1000 / num_instrs)
           << endl;
      cout << cache->NAME << " INSTRUCTION LOAD MPKI: "
           << ((double)(cache->roi_instr_miss[cpu][0]) * 1000 / num_instrs)
           << endl;
    }
  }
  ...
  if (cache->roi_access[cpu][1]) {
    ...
    cout << " RFO       ACCESS: " << setw(10) << cache->roi_access[cpu][1]
         ...
         << endl; //<< " T_ACCESS: " << setw(10) << cache->ACCESS[1] << " T_HIT:
                  //" << setw(10) << cache->HIT[1] << " T_MISS: " << setw(10) <<
                  //cache->MISS[1] << " T_MSHR_MERGED: " <<
                  //cache->MSHR_MERGED[1] << endl;
  }
  ...
  if (cache->roi_access[cpu][2]) {
    ...
    cout << " PREFETCH  ACCESS: " << setw(10) << cache->roi_access[cpu][2]
         ...
         << endl; //<< " T_ACCESS: " << setw(10) << cache->ACCESS[2] << " T_HIT:
                  //" << setw(10) << cache->HIT[2] << " T_MISS: " << setw(10) <<
                  //cache->MISS[2] << " T_MSHR_MERGED: " <<
                  //cache->MSHR_MERGED[2] << endl;

    cout << "AGUS PREFETCH L1 MISS: " << cache->pf_miss_l1 << endl;

    if (cache->cache_type == IS_L2C) {
      // Neelu: Instruction and Data loads.
      cout << cache->NAME << " DATA PREFETCH MPKI: "
           << ((double)(cache->roi_miss[cpu][2] -
                        cache->roi_instr_miss[cpu][2]) *
               1000 / num_instrs)
           << endl;
      cout << cache->NAME << " INSTRUCTION PREFETCH MPKI: "
           << ((double)(cache->roi_instr_miss[cpu][2]) * 1000 / num_instrs)
           << endl;
    }
  }
  ...
  if (cache->roi_access[cpu][3]) {
    ...
    cout << " WRITEBACK ACCESS: " << setw(10) << cache->roi_access[cpu][3]
         ...
         << endl; //<< " T_ACCESS: " << setw(10) << cache->ACCESS[3] << " T_HIT:
                  //" << setw(10) << cache->HIT[3] << " T_MISS: " << setw(10) <<
                  //cache->MISS[3] <<" T_MSHR_MERGED: " << cache->MSHR_MERGED[3]
                  //<< endl;
  }
  ...
  if (cache->roi_access[cpu][4]) {
    ...
    cout << " LOAD TRANSLATION ACCESS: " << setw(10)
         ...
         << endl; //<< " T_ACCESS: " << setw(10) << cache->ACCESS[4] << " T_HIT:
                  //" << setw(10) << cache->HIT[4] << " T_MISS: " << setw(10) <<
                  //cache->MISS[4] <<" T_MSHR_MERGED: " << cache->MSHR_MERGED[4]
                  //<< endl;
  }
  ...
  // cout << cache->NAME << " PF LOWER LEVEL TEST: " <<
  // cache->pf_lower_level_test << endl;
  cout << cache->NAME;
  ...
  cout << cache->NAME << " TIMELY PREFETCHES: " << setw(10) << cache->pf_useful
       << " LATE PREFETCHES: " << cache->pf_late
       << " DROPPED PREFETCHES: " << cache->pf_dropped << endl;
  // cout << cache->NAME << " PREFETCHES SAME FILL-ORIGIN LEVEL: " <<
  // cache->pf_same_fill_level << " DIFFERENT FILL-ORIGIN LEVEL: " <<
  // cache->pf_lower_fill_level << endl;

  //}
  cout << cache->NAME
       << " PREFETCHES SAME FILL-ORIGIN LEVEL: " << cache->pf_same_fill_level
       << " DIFFERENT FILL-ORIGIN LEVEL: " << cache->pf_lower_fill_level
       << endl;
  ...
  if (cache->cache_type == IS_L2C) {
    cout << cache->NAME << " Instructions Evicting Data "
         << cache->instr_evicting_data << endl;
    cout << cache->NAME << " Translations Evicting Data "
         << cache->transl_evicting_data << endl;
    cout << cache->NAME << " Data Evicting Data " << cache->data_evicting_data
         << endl;
    cout << cache->NAME << " Instructions Evicting Instructions "
         << cache->instr_evicting_instr << endl;
    cout << cache->NAME << " Translations Evicting Instructions "
         << cache->transl_evicting_instr << endl;
    cout << cache->NAME << " Data Evicting Instructions "
         << cache->data_evicting_instr << endl;
    cout << cache->NAME << " Instructions Evicting Translations "
         << cache->instr_evicting_transl << endl;
    cout << cache->NAME << " Translations Evicting Translations "
         << cache->transl_evicting_transl << endl;
    cout << cache->NAME << " Data Evicting Translations "
         << cache->data_evicting_transl << endl;
  }
  ...
  while (run_simulation) {
    ...
    for (int i = 0; i < NUM_CPUS; i++) {
      ...
      if (warmup_complete[i] &&
          current_core_cycle[i] % PHASE_SIZE_IN_CYCLES == 0) {
        // Listing the variables I need:
        // L1I: rq_to_cache, rq_merged, prefetch_miss, load_miss, pq_to_cache.
        cache_data_tag_accesses[0][phase_id] =
            0.2907293 * (ooo_cpu[i].L1I.RQ.TO_CACHE + ooo_cpu[i].L1I.RQ.MERGED +
                         ooo_cpu[i].L1I.sim_miss[i][2]);
        cache_tag_accesses[0][phase_id] =
            0.00244234 *
            (ooo_cpu[i].L1I.PQ.TO_CACHE + ooo_cpu[i].L1I.sim_miss[i][0]);

        // L1D: rq_to_cache, rq_merged, prefetch_miss, wq_to_cache, load_miss,
        // pq_to_cache, rfo_cache
        cache_data_tag_accesses[1][phase_id] =
            0.4359786 *
            (ooo_cpu[i].L1D.RQ.TO_CACHE + ooo_cpu[i].L1D.RQ.MERGED +
             ooo_cpu[i].L1D.sim_miss[i][2] + ooo_cpu[i].L1D.WQ.TO_CACHE);
        cache_tag_accesses[1][phase_id] =
            0.00556264 *
            (ooo_cpu[i].L1D.PQ.TO_CACHE + ooo_cpu[i].L1D.sim_miss[i][0] +
             ooo_cpu[i].L1D.sim_miss[i][1]);

        // L2C: rq_to_cache, prefetch_miss, wq_to_cache, total_miss, pq_to_cache
        cache_data_tag_accesses[2][phase_id] =
            0.4349426 *
            (ooo_cpu[i].L2C.RQ.TO_CACHE + ooo_cpu[i].L2C.sim_miss[i][2] +
             ooo_cpu[i].L2C.WQ.TO_CACHE + ooo_cpu[i].L2C.PQ.TO_CACHE);
        ...
        cache_data_tag_accesses[3][phase_id] =
            0.8522517 * (uncore.LLC.RQ.TO_CACHE + uncore.LLC.sim_miss[i][2] +
                         uncore.LLC.WQ.TO_CACHE + uncore.LLC.PQ.TO_CACHE);
        ...
        // DRAM: RQ/WQ row_buffer_hits/misses	NOTE: CAPTURING ONLY FOR CHANNEL
        // 0, BASED ON CURRENT CONFIGURATION.
        dram_accesses[phase_id] = 10.24 * (uncore.DRAM.RQ[0].ROW_BUFFER_HIT +
                                           uncore.DRAM.RQ[0].ROW_BUFFER_MISS +
                                           uncore.DRAM.WQ[0].ROW_BUFFER_HIT +
                                           uncore.DRAM.WQ[0].ROW_BUFFER_MISS);
// Prefetcher:
...
// IPCP:
        ...
        ip_table_tag_write[phase_id] =
            ip_table_tag_write_accesses * 0.000352568;
        ...
        cache_data_tag_accesses[4][phase_id] =
            0.001027806 *
            (ooo_cpu[i].ITLB.RQ.TO_CACHE + ooo_cpu[i].ITLB.RQ.MERGED);
        ...
        cache_data_tag_accesses[5][phase_id] =
            0.001027806 *
            (ooo_cpu[i].DTLB.RQ.TO_CACHE + ooo_cpu[i].DTLB.RQ.MERGED);

        cache_tag_accesses[5][phase_id] =
            0.000564828 * ooo_cpu[i].DTLB.sim_miss[i][0];

        // STLB: rq_to_cache, rq_merged, total_miss
        cache_data_tag_accesses[6][phase_id] =
            0.01775155 *
            (ooo_cpu[i].STLB.RQ.TO_CACHE + ooo_cpu[i].STLB.RQ.MERGED);
        ...
        interconnect_response[phase_id] =
            8 * 0.8 * (uncore.LLC.sim_access[i][3] + l2c_total_miss);
      ...
      if (stall_cycle[i] <= current_core_cycle[i]) {
        ...
        /*    if (ooo_cpu[i].ROB.occupancy < ooo_cpu[i].ROB.SIZE) {
                ...
                if (ooo_cpu[i].fetch_stall == 0)
                {
                    ...
                    //Neelu: Commenting this, to continue rereading trace file
           (for multi-core, one core might get free before another.
        ...
        // complete: inst. retired -> registers ready -> 
        ...
        /*
                Context-switch code
                //If a core is involved in context-switch, check if ROB
           occupancy of this core as well as another core is 0. Also, if they
           have reached same current_core_cycle, swap the contexts of those two
           cores
                */

        if (ooo_cpu[i].operating_index != -1) {
          cout << ooo_cpu[cs_file[ooo_cpu[i].operating_index].swap_cpu[0]]
                      .ROB.occupancy
               << " "
               << ooo_cpu[cs_file[ooo_cpu[i].operating_index].swap_cpu[1]]
                      .ROB.occupancy
               << " ";
          if (ooo_cpu[cs_file[ooo_cpu[i].operating_index].swap_cpu[0]]
                      .ROB.occupancy == 0 &&
              ooo_cpu[cs_file[ooo_cpu[i].operating_index].swap_cpu[1]]
                      .ROB.occupancy == 0 &&
              current_core_cycle[cs_file[ooo_cpu[i].operating_index]
                                     .swap_cpu[0]] ==
                  current_core_cycle[cs_file[ooo_cpu[i].operating_index]
                                         .swap_cpu[1]]) {
            cout << "entered with index " << ooo_cpu[i].operating_index
                 << " with cpu = " << i << endl;
            swap_context(cs_file[ooo_cpu[i].operating_index].swap_cpu[0],
                         cs_file[ooo_cpu[i].operating_index].swap_cpu[1]);
            cout << "IMPLEMENT CONTEXT SWITCH on cpu " << i << " cycle of cpu0"
                 << current_core_cycle[cs_file[ooo_cpu[i].operating_index]
                                           .swap_cpu[0]]
                 << " CPU1 "
                 << current_core_cycle[cs_file[ooo_cpu[i].operating_index]
                                           .swap_cpu[1]]
                 << endl;
            ooo_cpu[cs_file[ooo_cpu[i].operating_index].swap_cpu[0]]
                .context_switch = 0;
            ooo_cpu[cs_file[ooo_cpu[i].operating_index].swap_cpu[1]]
                .context_switch = 0;
            cout << "Done! " << cs_file[ooo_cpu[i].operating_index].swap_cpu[0]
                 << cs_file[ooo_cpu[i].operating_index].swap_cpu[1] << endl;
            int index =
                ooo_cpu[i].operating_index; // operating index will be modified
            ooo_cpu[cs_file[index].swap_cpu[0]].operating_index = -1;
            ooo_cpu[cs_file[index].swap_cpu[1]].operating_index = -1;

            cout << "operatingINDEX"
                 << ooo_cpu[cs_file[index].swap_cpu[0]].operating_index << " "
                 << ooo_cpu[cs_file[index].swap_cpu[1]].operating_index << endl;
          }
        }
      }
