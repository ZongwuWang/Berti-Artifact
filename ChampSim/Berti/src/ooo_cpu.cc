...
void O3_CPU::initialize_core() {
  ...
  cout << "Running both PERFECT_L1D and PRACTICAL_PERFECT_L1D. Can run only "
          "one of these."
       << endl;
  assert(0);
#endif
  ...
  cout << "Running both PERFECT_L2C and PRACTICAL_PERFECT_L2C. Can run only "
          "one of these."
       << endl;
  assert(0);
#endif
  ...
  while (continue_reading) {
    ...
    if (knob_cloudsuite) {
      ...
      if (!fread(&current_cloudsuite_instr, instr_size, 1, trace_file)) {
        ...
        // trace: " << trace_string << endl; TRACE_ENDS_STOP = 1; /*@Vasudha -
        // STOP simulation once trace file ends*/
        // close the trace file and re-open it
        pclose(trace_file);
        // Neelu: Commenting return on trace file end, need to keep reading
        // trace for equal number of instructions simulation for all cores in
        // multi-core. return; /*@Vasudha */
      ...
      if (ooo_cpu[cpu].context_switch) {
        // cout << "context_switch = " << cpu <<"
        // cycle="<<current_core_cycle[cpu]<<"ROB-occupancy = "<< ROB.occupancy
        // <<  endl;
      ...
      if (!fread(&trace_read_instr, instr_size, 1, trace_file)) {
        ...
        ////cout << arch_instr.instr_id <<" "<<arch_instr.ip<<" "<<
        ///arch_instr.is_branch<<" "<<arch_instr.branch_taken<<" "<<cpu<<endl;
        for (uint32_t i = 0; i < MAX_INSTR_DESTINATIONS; i++) {
          arch_instr.destination_registers[i] =
        ...
        if (IFETCH_BUFFER.occupancy < IFETCH_BUFFER.SIZE) {
          ...
          /*   if (ROB.occupancy < ROB.SIZE) {
                 ...
                 num_reads++;
                // //cout << "cycle="<<current_core_cycle[cpu] << "
             cpu="<<cpu<<" addto rob -" <<ROB.occupancy << " ";

                 // branch prediction
                 if (arch_instr.is_branch) {

                     //DP( if (warmup_complete[cpu]) {
                     //cout << "[BRANCH] instr_id: " << instr_unique_id << " ip:
             " << hex << arch_instr.ip << dec << " taken: " <<
             +arch_instr.branch_taken << endl; });
                     ...
                     /*
                     uint8_t branch_prediction;
                     // for faster simulation, force perfect prediction during
             the warmup
                     // note that branch predictor is still learning with real
             branch results if (all_warmup_complete == 0) branch_prediction =
             ...
             predict_branch(arch_instr.ip);
                     ...
                     if (arch_instr.branch_taken != branch_prediction) {
                         //if(false) { // this simulates perfect branch
             prediction branch_mispredictions++;

                         total_rob_occupancy_at_branch_mispredict +=
             ROB.occupancy;

                         //DP( if (warmup_complete[cpu]) {
                         //cout << "[BRANCH] MISPREDICTED instr_id: " <<
             instr_unique_id << " ip: " << hex << arch_instr.ip << dec;
             ...
             predicted: " << +branch_prediction << endl; });
                         ...
                         // and stall any additional fetches until the branch is
             executed fetch_stall = 1;

                         ROB.entry[rob_index].branch_mispredicted = 1;
                     }
                     else {
                         if (branch_prediction == 1) {
                             // if we are accurately predicting a branch to be
             taken, then we can't possibly fetch down that path this cycle,
                             // so we have to wait until the next cycle to fetch
             those instrs_to_read_this_cycle = 0;
                         }

                         //DP( if (warmup_complete[cpu]) {
                         //cout << "[BRANCH] PREDICTED    instr_id: " <<
             instr_unique_id << " ip: " << hex << arch_instr.ip << dec;
                         //cout << " taken: " << +arch_instr.branch_taken << "
             predicted: " << +branch_prediction << endl; });
                     }
...
void O3_CPU::fetch_instruction() {
  ...
  // mispredict penalty
  ////////cout <<"prev, fetch_stall="<<fetch_stall<<"
  ///"<<current_core_cycle[cpu]<<" "<<fetch_resume_cycle<<endl;
  if ((fetch_stall == 1) && (current_core_cycle[cpu] >= fetch_resume_cycle) &&
      (fetch_resume_cycle != 0)) {
    ////cout<<"fetch_resume at "<< fetch_resume_cycle<<" " ;
    fetch_stall = 0;
    fetch_resume_cycle = 0;
    ////cout <<"****noww, fetch_stall="<<fetch_stall<<"
    ///"<<current_core_cycle[cpu]<<" "<<fetch_resume_cycle<<endl;
  }
...
/*uint32_t read_index = (ROB.last_read == (ROB.SIZE-1)) ? 0 : (ROB.last_read +
1); for (uint32_t i=0; i<FETCH_WIDTH; i++) {
...
<< " ROB.tail: " << ROB.tail << endl; assert(0);
        }
    }
#endif
    ...
    if (knob_cloudsuite)
        trace_packet.address = ((ROB.entry[read_index].ip >> LOG2_PAGE_SIZE) <<
9) | ( 256 + ROB.entry[read_index].asid[0]); else trace_packet.address =
ROB.entry[read_index].ip >> LOG2_PAGE_SIZE; trace_packet.full_addr =
ROB.entry[read_index].ip; trace_packet.instr_id =
ROB.entry[read_index].instr_id; trace_packet.rob_index = read_index;
...
/*    uint32_t fetch_index = (ROB.last_fetch == (ROB.SIZE-1)) ? 0 :
   (ROB.last_fetch + 1);
        ////DP ( if (warmup_complete[cpu]) {
   //*******************************************************************************************
              //  //cout << " and fetch instr id:"<<
   ROB.entry[fetch_index].instr_id << endl; }); for (uint32_t i=0;
   i<FETCH_WIDTH; i++) {

        // fetch is in-order so it should be break
        // @Vishal: TODO see if we can make out of order fetch as
   schedule_instruction is in order.
   ...
   (ROB.entry[fetch_index].event_cycle > current_core_cycle[cpu]))
        ...
        if (ROB.entry[fetch_index].fetched) {
            if (fetch_index == ROB.head)
                ...
                //cout << "fetch_index: " << fetch_index << " ROB.head: " <<
   ROB.head << " ROB.tail: " << ROB.tail << endl; assert(0);
            }
        }
        ...
        //@Vishal: VIPT virtual address will be sent to L1I instead of physical
   address
        ...
        fetch_packet.full_addr = ROB.entry[fetch_index].instruction_pa; //Neelu:
   Removed comment ending.
...
// DP ( if (warmup_complete[cpu]) {
//          //cout << "Exit fetch_instruction() with read instr id:"<<
//          ROB.entry[read_index].instr_id << "fetch instr_id:"<<
//          ROB.entry[fetch_index].instr_id<< endl; });
...
void O3_CPU::decode_and_dispatch() {
  ...
  for (uint32_t i = 0; i < DECODE_BUFFER.SIZE; i++) {
    ...
    if (((!warmup_complete[cpu]) && (ROB.occupancy < ROB.SIZE)) ||
      ...
      if (DECODE_BUFFER.entry[DECODE_BUFFER.head].btb_miss == 1 &&
        ...
        if (branch_type == BRANCH_DIRECT_JUMP ||
          ...
          // assert(DECODE_BUFFER.entry[DECODE_BUFFER.head].ip + 4 !=
    ...
    //   if (ROB.occupancy > (0.25 * ROB.SIZE)) {
    //     if (ROB.entry[ROB.head].ip != ROB.last_block_ip || ROB.last_block_level != CORE_BUFFER::quarter) {
    //         ROB.last_block_ip = ROB.entry[ROB.head].ip;
    //         ROB.last_block_level = CORE_BUFFER::quarter;
    //         // send_prefetch_info_to_Cache("quarter", "ip", ...);
    //         printf("%u: Current ROB occupancy [%u], ip [%lu]\n", __LINE__, ROB.occupancy, ROB.entry[ROB.head].ip);
    //     }
    //   } else if ( ROB.occupancy > (0.5 * ROB.SIZE)) {
    //     if (ROB.entry[ROB.head].ip != ROB.last_block_ip || ROB.last_block_level != CORE_BUFFER::half) {
    //         ROB.last_block_ip = ROB.entry[ROB.head].ip;
    //         ROB.last_block_level = CORE_BUFFER::half;
    //         // send_prefetch_info_to_Cache("half", "ip", ...);
    //         printf("%u: Current ROB occupancy [%u], ip [%lu]\n", __LINE__, ROB.occupancy, ROB.entry[ROB.head].ip);
    //     }
    //   } else if ( ROB.occupancy > (0.75 * ROB.SIZE)) {
    //     if (ROB.entry[ROB.head].ip != ROB.last_block_ip || ROB.last_block_level != CORE_BUFFER::quarter3) {
    //         ROB.last_block_ip = ROB.entry[ROB.head].ip;
    //         ROB.last_block_level = CORE_BUFFER::quarter3;
    //         // send_prefetch_info_to_Cache("quarter3", "ip", ...);
    //         printf("%u: Current ROB occupancy [%u], ip [%lu]\n", __LINE__, ROB.occupancy, ROB.entry[ROB.head].ip);
    //     }
    //   }
...
int O3_CPU::prefetch_code_line(uint64_t pf_v_addr) {
  ...
  if (L1I.PQ.occupancy < L1I.PQ.SIZE) {
    ...
    /*  uint64_t pf_pa = (va_to_pa(cpu, 0, pf_v_addr, pf_v_addr>>LOG2_PAGE_SIZE,
     * 1) & (~((1 << LOG2_PAGE_SIZE) - 1))) | (pf_v_addr & ((1 <<
     * LOG2_PAGE_SIZE) - 1)); */
...
void O3_CPU::schedule_instruction() {
  ...
  ////DP ( if (warmup_complete[cpu]) {
  /////*******************************************************************************************
  //          //cout << "Entered schedule_instruction() with instr id:"<<
  //          ROB.entry[limit].instr_id << endl; });
  num_searched = 0;
  if (ROB.head < limit) {
...
void O3_CPU::reg_dependency(uint32_t rob_index) {
  ...
  for (uint32_t i=0; i<NUM_INSTR_SOURCES; i++) {
      if (ROB.entry[rob_index].source_registers[i]) {
          //cout << "[ROB] " << __func__ << " instr_id: " <<
  ROB.entry[rob_index].instr_id << " is_memory: " <<
...
void O3_CPU::execute_instruction() {
  ...
  while (exec_issued < EXEC_WIDTH) {
    if (RTE0[RTE0_head] < ROB_SIZE) {
      ...
      ////DP ( if (warmup_complete[cpu]) {
      /////*******************************************************************************************
      //  //cout << "Entered execute_instruction() with  RTE0 with instr_id: "<<
      //  ROB.entry[exec_index].instr_id<< endl; });
  ...
  while (exec_issued < EXEC_WIDTH) {
    if (RTE1[RTE1_head] < ROB_SIZE) {
      ...
      // //DP ( if (warmup_complete[cpu]) {
      // //*******************************************************************************************
      //   //cout << "Entered execute_instruction() with  RTE1 with instr_id:
      //   "<< ROB.entry[exec_index].instr_id<< endl; });
      if (ROB.entry[exec_index].event_cycle <= current_core_cycle[cpu]) {
        do_execution(exec_index);
...
void O3_CPU::do_execution(uint32_t rob_index) {
  ...
  // DP (if (warmup_complete[cpu]) {
  // cout << "[ROB] " << __func__ << " non-memory instr_id: " <<
  // ROB.entry[rob_index].instr_id; cout << " event_cycle: " <<
...
void O3_CPU::schedule_memory_instruction() {
  ...
  // //DP ( if (warmup_complete[cpu]) {
  // //*******************************************************************************************
  //           //cout << "Entered schedule_memory_instruction() with
  //           next_schedule instr_id: "<< ROB.entry[limit].instr_id<< endl; });
  num_searched = 0;
  if (ROB.head < limit) {
    for (uint32_t i = ROB.head; i < limit; i++) {
...
void O3_CPU::add_load_queue(uint32_t rob_index, uint32_t data_index) {
  ...
  /*int accessed_offset = (ROB.entry[rob_index].source_memory[data_index] >> 6)
  & 0x3F; if(offsets_accessed_by_ip[cpu].find(ROB.entry[rob_index].ip) ==
  offsets_accessed_by_ip[cpu].end())
  {
  ...
  if (forwarding_index != SQ.SIZE) { // we have a store-to-load forwarding
    ...
    if ((SQ.entry[forwarding_index].fetched == COMPLETED) &&
      ...
      //fetched LQ.entry[lq_index].physical_address =
      // (SQ.entry[forwarding_index].physical_address & ~(uint64_t) ((1 <<
...
void O3_CPU::operate_lsq() {
  ...
  LOG2_PAGE_SIZE) << 9) | SQ.entry[sq_index].asid[1]; else data_packet.address =
  SQ.entry[sq_index].virtual_address >> LOG2_PAGE_SIZE; data_packet.full_addr =
  SQ.entry[sq_index].virtual_address; data_packet.instr_id =
  SQ.entry[sq_index].instr_id; data_packet.rob_index =
  SQ.entry[sq_index].rob_index; data_packet.ip = SQ.entry[sq_index].ip;
              ...
              //DP (if (warmup_complete[cpu]) {
              //cout << "[RTS0] " << __func__ << " instr_id: " <<
  SQ.entry[sq_index].instr_id << " rob_index: " << SQ.entry[sq_index].rob_index
  ...
  LOG2_PAGE_SIZE) << 9) | LQ.entry[lq_index].asid[1]; else data_packet.address =
  LQ.entry[lq_index].virtual_address >> LOG2_PAGE_SIZE; data_packet.full_addr =
  LQ.entry[lq_index].virtual_address; data_packet.instr_id =
  LQ.entry[lq_index].instr_id; data_packet.rob_index =
  LQ.entry[lq_index].rob_index; data_packet.ip = LQ.entry[lq_index].ip;
              ...
              //DP (if (warmup_complete[cpu]) {
              //cout << "[RTL0] " << __func__ << " instr_id: " <<
  LQ.entry[lq_index].instr_id << " rob_index: " << LQ.entry[lq_index].rob_index
  << " is popped to RTL0";
  ...
  lq_index, LQ.entry[lq_index].data_index);
...
void O3_CPU::complete_instr_fetch(PACKET_QUEUE *queue, uint8_t is_it_tlb) {
  ...
  // Neelu: Todo: IFETCH_BUFFER entries, mark translated and fetched. Since TLB
  // requests will not be handled here due to VIPT, I am not
  ...
  // Neelu: Marking IFETCH_BUFFER entries translated and fetched and then
  // returning.
...
void O3_CPU::handle_o3_fetch(PACKET *current_packet, uint32_t cache_type)
{
    ...
    if (cache_type == 0) { // DTLB
        ...
        if (current_packet->type == RFO) {
            SQ.entry[sq_index].physical_address = (current_packet->data_pa <<
LOG2_PAGE_SIZE) | (SQ.entry[sq_index].virtual_address & ((1 << LOG2_PAGE_SIZE) -
1)); // translated address SQ.entry[sq_index].translated = COMPLETED;
...
dec << " store_merged: " << +current_packet->store_merged;
            ...
            LQ.entry[lq_index].physical_address = (current_packet->data_pa <<
LOG2_PAGE_SIZE) | (LQ.entry[lq_index].virtual_address & ((1 << LOG2_PAGE_SIZE) -
1)); // translated address LQ.entry[lq_index].translated = COMPLETED;
...
(LQ.entry[lq_index].physical_address>>LOG2_BLOCK_SIZE);
            //cout << " full_addr: " << LQ.entry[lq_index].physical_address <<
dec << " remain_mem_ops: " << ROB.entry[rob_index].num_mem_ops;
...
void O3_CPU::retire_rob() {
  for (uint32_t n = 0; n < RETIRE_WIDTH; n++) {
    ...
    if (current_core_cycle[cpu] - ROB.entry[ROB.head].stall_begin_cycle > ROB_DELAY_THRESHOLD) {
      l1d_prefetcher_allocate_operate(ROB.entry[ROB.head].ip, ROB.entry[ROB.head].virtual_address, current_core_cycle[cpu] - ROB.entry[ROB.head].stall_begin_cycle, cpu);
    }
    #endif
